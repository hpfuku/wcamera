...
function renderResult(name, content) {
  ...
  html += '<h3>' + name + '</h3>'; //FileName
  html += '<pre>' + content + '</pre>';
  elem.innerHTML = html;
  const parsed = JSON.parse(content);
  const sheetData = parsed["Sheet1"];
  
  for (var i=0; i<sheetData.length; i++){

    //alert(sheetData.length); //3

    //var rd = r[i];
  
    r.push(parsed["Sheet1"][i]["氏名"]);
    r.push(parsed["Sheet1"][i]["住所"]);
    r.push(parsed["Sheet1"][i]["郵便番号"]);
    //alert(r);
